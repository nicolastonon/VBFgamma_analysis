...
parser.add_argument('--label-size', type=float, default=0.03, help='Parameter name label size')
...
plot.ModTDRStyle(l=args.left_margin, b=0.1, r=0.005, width=(1100 if args.checkboxes else 900), height=args.height)

# We will assume the first POI is the one to plot
POIs = [ele['name'] for ele in data['POIs']]
POIs_fit = [ele['fit'] for ele in data['POIs']]
...
for page in xrange(n):
    canv = ROOT.TCanvas(os.path.relpath(args.output), os.path.relpath(args.output))
    ...
    if args.checkboxes:
        ...
        pads = plot.MultiRatioSplitColumns([x_pulls]+[(1.-x_pulls)/nPOIs for j in range(nPOIs)], [0. for j in range(nPOIs+2)], [0. for j in range(nPOIs+2)])
        for j in range(nPOIs-1):
            pads[j+2].SetGrid(1, 0)
    else:
        x_pulls=0.4
        pads = plot.MultiRatioSplitColumns([x_pulls]+[(1.-x_pulls)/nPOIs for j in range(nPOIs-1)], [0. for j in range(nPOIs+1)], [0. for j in range(nPOIs+1)])
        for j in range(nPOIs-1):
            pads[j+2].SetGrid(1, 0)
    pads[0].SetGrid(1, 0)
    pads[0].SetTickx(1)
    ...
    for p in xrange(n_params):
        ...
        for j in range(nPOIs):
            ...
            max_impact[j] = max(
                max_impact[j], abs(imp[j][1] - imp[j][0]), abs(imp[j][2] - imp[j][1]))
    ...
    if externalPullDef:
        plot.Set(h_pulls.GetXaxis(), TitleSize=0.04, LabelSize=args.label_size, Title=CP.returnTitle(args.pullDef))
    else:
        plot.Set(h_pulls.GetXaxis(), TitleSize=0.04, LabelSize=args.label_size, Title='(#hat{#theta}-#theta_{0})/#Delta#theta')

    plot.Set(h_pulls.GetYaxis(), LabelSize=args.label_size, TickLength=0.)
    ...
    for j, poi in enumerate(POIs):
        ...
        h_impact = ROOT.TH2F(
            "impacts_%s" % poi, "impacts", 6, -max_impact[j] * 1.1, max_impact[j] * 1.1, n_params, 0, n_params)
    ...
    for j in range(nPOIs):
        pads[j+1].cd()
        g_impacts_hi[j].SetFillColor(plot.CreateTransparentColor(hi_color[method], alpha))
        g_impacts_hi[j].Draw('2SAME')
        g_impacts_lo[j].SetFillColor(plot.CreateTransparentColor(lo_color[method], alpha))
        g_impacts_lo[j].Draw('2SAME')
        pads[j+1].RedrawAxis()


    legend = ROOT.TLegend(0.01, 0.01, args.left_margin*0.6, 0.08, '', 'NBNDC')
    legend.SetNColumns(2)
    legend.AddEntry(g_impacts_hi[0], '+1#sigma Impact', 'F')
    legend.AddEntry(g_pulls, 'Pull', 'LP')
    legend.AddEntry(g_impacts_lo[0], '-1#sigma Impact', 'F')
    legend.Draw()
    ...
    plot.DrawCMSLogo(pads[0], 'CMS', args.cms_label, 0, args.left_margin+0.1, 0.00, 0.00)
    for i, (POI, POI_fit) in enumerate(zip(POIs, POIs_fit)):
        ...
        if not args.blind:
            plot.DrawTitle(pads[i+1], '#hat{%s} = %s^{#plus%s}_{#minus%s}%s' % (
